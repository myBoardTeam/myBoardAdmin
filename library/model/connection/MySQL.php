...
class MySQL extends AbstractMessageLog {
	...
	function __destruct() {
		...
		if ($this->database_connection)
			...
			$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Erro ao finalizar conexão com o Servidor MySQL.", mysql_error());
	}
	...
	public function connect( ) {
		...
		if (!$this->database_connection)
			$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Erro ao conectar ao Servidor MySQL.", mysql_error());
		else
			if (!mysql_select_db($this->getDatabase(), $this->database_connection))
				$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Erro ao selecionar Base de Dados.", mysql_error());
	}
		...
		if ( $q != "" ) {
			...
			if ($this->database_connection) {
				if (!$this->resultset = mysql_query( $q, $this->database_connection ))
					$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Erro ao executar comando no Banco de Dados. Verifique a sintaxe.", mysql_error());
				else {
					$this->query = $q;
				...
				$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Impossivel executar comando no Banco de Dados. Conexão inexistente.", $q);
		} else
			$this->addMessage($this->get_class(), $fname, MB_ERROR, MB_HIDDEN, "Comando de execução do Banco de Dados inválido. Impossível executar comando vazio.");
		return(false);
	}
		...
		if ( $this->query != "" ) {
			if ($type == DB_INSERT) {
				if (!$return = mysql_insert_id()) {
					$this->addMessage($this->get_class(), $fname, MB_WARNING, MB_HIDDEN, "Não existem resultados de inclusão relacionados.");
				} else return($return);
				...
				else $this->addMessage($this->get_class(), $fname, MB_WARNING, MB_HIDDEN, "Não existem resultados relacionados.");
			}
		}
		return(false);
	}
